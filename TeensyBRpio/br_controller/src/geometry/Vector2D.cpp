...
template <typename Unit>
Vector2D<Unit>::Vector2D(number_t x, number_t y) : x(x), y(y) {}

template <typename Unit>
number_t Vector2D<Unit>::norm() const {
...
template <typename Unit>
void Vector2D<Unit>::operator*=(number_t factor) {
    x *= factor;
    y *= factor;
}
template <typename Unit>
...
template <typename Unit>
Vector2D<Unit> Vector2D<Unit>::operator*(number_t factor) const {
...
template <typename Unit>
number_t Vector2D<Unit>::distance(Vector2D<Unit> a, Vector2D<Unit> b) {
    return (b - a).norm();
}
template <typename Unit>
Angle Vector2D<Unit>::angle(Vector2D<Unit> a, Vector2D<Unit> b) {
    number_t prod = dot(a, b);
    number_t det = a.x * b.y - a.y * b.x; // Determinant
    if (det == 0 && dot == 0) {
        return 0;
    }
...
template <typename Unit>
number_t Vector2D<Unit>::dot(Vector2D<Unit> a, Vector2D<Unit> b) {
...
#define __SPECIALIZE(UNIT)                                                                                                                           \
    template class Vector2D<UNIT>;                                                                                                                   \
                                                                                                                                                     \
    template <>                                                                                                                                      \
    Vector2D<UNIT> clamp<Vector2D<UNIT>>(Vector2D<UNIT> value, number_t minBound, number_t maxBound) {                                                     \
