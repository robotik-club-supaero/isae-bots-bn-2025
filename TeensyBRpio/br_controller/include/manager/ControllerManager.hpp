 ...
 */
template <Actuators TActuators, CanControl<TActuators> TController, PositionFeedback TFeedback>
class ControllerManager
  ...
  public:
     ...
     */
    ControllerManager(duration_t updateInterval, TController controller, TActuators actuators, TFeedback feedback);

    ControllerManager(duration_t minUpdateInterval, duration_t maxUpdateInterval, TController controller, TActuators actuators,
                      TFeedback feedback);

    /// Initializes the manager with the default values from the configuration file.
    ControllerManager()
        requires  std::is_default_constructible_v<TController> && std::is_default_constructible_v<TFeedback> && std::is_constructible_v<TActuators, const TFeedback &>
        : ControllerManager(create()) {}

  private:
    /// Initializes the manager with the default values from the configuration file.
    ControllerManager(TActuators actuators, TFeedback feedback)
        requires std::is_default_constructible_v<TController>
        : ControllerManager(UPDATE_INTERVAL, TController(), std::move(actuators), std::move(feedback)) {}
  ...
  public:
    ...
    bool sendOrder(Fun &&order) {
        if (isActive()) {
            std::forward<Fun>(order)(m_controller, m_feedback.getRobotPosition());
            return true;
        } else {
  ...
  private:
    using Self = ControllerManager<TActuators, TController, TFeedback>;
