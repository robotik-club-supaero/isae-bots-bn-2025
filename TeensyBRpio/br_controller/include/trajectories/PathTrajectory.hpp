...
class BezierCurvesGenerator {
  public:
    using size_type = SmallDeque<Point2D<Meter>>::size_type;

    BezierCurvesGenerator(std::optional<Angle> initialDirection, std::optional<Angle> finalDirection, SmallDeque<Point2D<Meter>> &&points);

    BezierCurve<4> next();
    bool hasNext() const;

    const SmallDeque<Point2D<Meter>> &getPoints() const;
    size_type getCurrentCurveIndex() const;
    const std::optional<Angle> &getPreferredFinalDirection() const;

  private:
    SmallDeque<Point2D<Meter>> &getPoints();

    SmallDeque<Point2D<Meter>> m_points;
    Vector2D<Meter> m_initialDirection;
    std::optional<Angle> m_finalDirection;
    friend class PathTrajectory;
};
 ...
 */
class PathTrajectory final : public MultiCurveTrajectory<BezierCurve<4>, BezierCurvesGenerator> {
  ...
  public:
    ...
    PathTrajectory(std::optional<Angle> initialDirection, std::optional<Angle> finalDirection, SmallDeque<Point2D<Meter>> points);

    /// The crosspoints the trajectory must pass through. This only includes the points that were not reached yet.
    const SmallDeque<Point2D<Meter>> &getPathPoints() const;
